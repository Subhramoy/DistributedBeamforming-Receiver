...
bfAlg         = 'basic';  % options are: basic, RWA
%% Comms parameters
Atx = 3;  % Antenna Gain at the transmitter (in dB)
Noise = 105;  % Noise floor (in dBm)
SINRdb = 20.5;  % SNR target at the receiver (in dB)
P_ch_desired = 150;  % Channel losses (in dB)
...
%     if strcmp(bfAlg,'basic') || (i<=30) % Channel inversion
%         beamWeight = f_BFPayload(channelEst);
%     elseif strcmp(bfAlg,'RWA')  % Channel inversion + Waterfilling
%         [beamWeight,~] = f_BFPayload_WF(channelEst,DeltaP,inc,SINRdb,Atx,gain,Noise,BBPowMax,BBPowPayload);
%     end
    ...
    if verbose
        ...
        for id = 1:numTxAntennas
            fprintf('H(%d) = %.5f + %.5fj | ',id,real(channelEst(id)),imag(channelEst(id)));
        end
        fprintf('\n');
    end
